...
// Set the bot TOKEN
$bot_id = "1279873723:AAEjyOf7og7FWiK0IAXJ9vyPqqwItv-6HEs"; #manotoapibot
...
else if ( stristr ( $text , 'series' ) == TRUE )
{
    ...
    $keyb = $telegram -> buildInlineKeyBoard ( $option );
    $telegram -> sendPhoto ( [ 'chat_id' => $chat_id , 'photo' => $ImgIxUrl , 'reply_markup' => $keyb , 'caption' => $Description , 'parse_mode' => 'HTML' ] );
    #$telegram->sendMessage(['chat_id' => $chat_id, 'text' => $videoM3u8Url]);
}
    ...
    if ( $status == "1" ) {
        ...
        $keyb = $telegram -> buildInlineKeyBoard ( $option );
        $telegram -> sendPhoto ( [ 'chat_id' => $chat_id , 'reply_markup' => $keyb , 'photo' => $ImgIxUrl , 'caption' => tr_num ( $Description , 'fa' ) , 'parse_mode' => 'markdown' ] );
        #unlink ( "epiphoto.jpg" );
        if ( $sendhlsurl == "1" ) {
            $telegram -> sendMessage ( [ 'chat_id' => $chat_id , 'text' => $videoM3u8Url ] );
        }
    ...
    $array = json_decode ( file_get_contents ( "https://dak1vd5vmi7x6.cloudfront.net/api/v1/publicrole/schedulemodule/schedule?from=" . $ENDATE . "T" . $ENTIME . ":" . $minute . ":00.000Z&to=" . $ENDATE . "T" . $hourplus . ":00.000Z" ) , TRUE );
    ...
    if ( ! empty( $inline_query_text ) ) {
        if ( $inline_query_text == "l" ) {
            ...
            $scheduleItemID = $array[ 'details' ][ 'list' ][ '0' ][ 'scheduleItemID' ];
            $dateUTC = $array[ 'details' ][ 'list' ][ "0" ][ 'dateUTCRoundedDownToFiveMinutes' ];
            $showTitle = $array[ 'details' ][ 'list' ][ "0" ][ 'showTitle' ];
            $episodeNumberen = $array[ 'details' ][ 'list' ][ "0" ][ 'episodeNumber' ];
            $seasonNumberen = $array[ 'details' ][ 'list' ][ "0" ][ 'seasonNumber' ];
            $showID = $array[ 'details' ][ 'list' ][ "0" ][ 'showID' ];
            $episodeID = $array[ 'details' ][ 'list' ][ "0" ][ 'episodeID' ];
            $schedulecurrentHouseNumber = $array[ 'details' ][ 'list' ][ "0" ][ 'currentHouseNumber' ];
            $portraitImgIxUrl = $array[ 'details' ][ 'list' ][ "0" ][ 'portraitImgIxUrl' ];
